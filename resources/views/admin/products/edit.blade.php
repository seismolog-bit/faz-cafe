...
<form class="mb-9" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" method="post">
    @method('patch')
    @csrf
    ...
    <div class="row g-5">
        <div class="col-12 col-xl-8">
            <input class="form-control mb-5" type="text" placeholder="Masukan nama produk" required name="name" value="{{ $product->name }}"/>
            <div class="mb-6">
                <h4 class="mb-3"> Detail</h4>
                <textarea name="detail" id="summernote" required placeholder="Masukan detail produk">{{ $product->detail }}</textarea>
            </div>
        </div>
                            ...
                            <div class="row g-3">
                                <div class="col-12 col-sm-6 col-xl-12">
                                    <div class="col-12 col-sm-6 col-xl-12">
                                        <div class="mb-xl-4">
                                            ...
                                            <input class="form-control" type="number" name="price" placeholder="contoh: 35000" value="{{$product->price}}" required />
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                ...
                                <div class="col-12 col-sm-6 col-xl-12">
                                    <div class="mb-xl-4">
                                        ...
                                        <span class="mb-2"><small>*Khusus paket billiard (satuan menit)</small></span>
                                        <input class="form-control" type="number" name="duration" value="{{ $product->duration }}" placeholder="contoh: 60" />
                                    </div>
                                </div>
                                <div class="col-12 col-sm-6 col-xl-12">
                                    <div class="mb-4">
                                        ...
                                        single="single" data-options='{"removeItemButton":true,"placeholder":true}'>
                                            ...
                                            @foreach ($items as $item)
                                                <option value="{{ $item->id }}" {{ $product->item_id == $item->id ? 'selected' : '' }}>{{ $item->name }} ({{ $item->stock . $item->unit }} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
