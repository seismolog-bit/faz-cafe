...
<div class="container">
    ...
    <div class="row justify-content-between">
        <div class="col-lg-7 col-xl-7">
            <div id="members"
                data-list='{"valueNames":["product","category","price","qty","total","status"],"page":15,"pagination":true}'>
                ...
                <div class="mx-n4 mx-lg-n6 px-4 px-lg-6 mb-d bg-white border-y border-300 mt-2 position-relative top-1">
                    <div class="table-responsive scrollbar ms-n1 ps-1">
                        <table class="table table-sm fs--1 mb-0">
                            <thead>
                                <tr>
                                    <th class="sort align-middle" scope="col" data-sort="product"
                                        style="min-width: 200px;">PRODUK</th>
                                    {{-- <th class="sort align-middle" scope="col" data-sort="category"
                                        style="min-width: 100px;">KATEGORI</th> --}}
                                    <th class="sort align-middle text-end" scope="col" data-sort="price"
                                        style="min-width: 100px;">HARGA</th>
                                    <th class="sort align-middle text-center" scope="col" data-sort="qty"
                                        style="min-width: 50px;"> QTY</th>
                                    <th class="sort align-middle text-end" scope="col" data-sort="total"
                                        style="min-width: 100px;"> TOTAL</th>
                            ...
                            <tbody class="list" id="table-latest-review-body">
                                ...
                                <tr class="hover-actions-trigger btn-reveal-trigger position-static">
                                    ...
                                    <td class="price white-space-nowrap text-end">
                                        {{ number_format($item->price) }}
                                    </td>
                                    <td class="qty white-space-nowrap text-center">
                                        {{ $item->qty }}
                                    </td>
                                    <td class="total white-space-nowrap text-end">
                                        {{ number_format($item->grand_total) }}
                                    </td>
                                    <td
                                        class="status white-space-nowrap text-center {{ !$item->payment ? 'text-danger' : '' }}">
                                        {{ $item->payment ? $item->payment_method : 'Belum bayar' }}
                                    </td>
                ...
                <div class="card-body">
                    ...
                    <div class="border-dashed border-bottom mt-4">
                        <div class="d-flex justify-content-between mb-2">
                            <h5 class="text-900 fw-semi-bold">ID Card: </h5>
                            <h5 class="text-900 fw-semi-bold">{{ $order->card->code }}</h5>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                    ...
                    <div class="border-dashed border-bottom mt-4">
                        <div class="d-flex justify-content-between mb-2">
                            <h5 class="text-900 fw-semi-bold">Terbayar: </h5>
                            <h5 class="text-900 fw-semi-bold">{{ number_format($order->order_items->where('payment',
                                '1')->sum('total')) }}</h5>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <h5 class="text-900 fw-semi-bold">Belum dibayar: </h5>
                            <h5 class="text-900 fw-semi-bold text-danger">{{
                                number_format($order->order_items->where('payment', '0')->sum('total')) }}</h5>
                        </div>
                    </div>
